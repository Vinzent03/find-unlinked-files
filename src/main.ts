...
export default class FindOrphanedFilesPlugin extends Plugin {
  ...
  async findEmptyFiles() {
    ...
    for (const file of files) {
      ...
      if (frontmatter) {
        ...
        if (
          (cache.frontmatterPosition ?? frontmatter.position).end.line ==
          lines - 1
        ) {
          emptyFiles.push(file);
        }
  ...
  findOrphanedFiles(dir?: string) {
    ...
    markdownFiles.forEach((markFile: TFile) => {
      ...
      this.app.metadataCache.getFileCache(markFile).links?.length &&
        this.app.metadataCache.getFileCache(markFile).links.map((fml: any) => {
          const txt = this.app.metadataCache.getFirstLinkpathDest(
            getLinkpath(fml.link),
            markFile.path
          );
          if (txt != null) links.push(txt.path);
        });

      this.app.metadataCache.getFileCache(markFile).frontmatterLinks?.length &&
        this.app.metadataCache
          .getFileCache(markFile)
          .frontmatterLinks.map((fml: any) => {
            const txt = this.app.metadataCache.getFirstLinkpathDest(
              getLinkpath(fml.link),
              markFile.path
            );
  ...
  async deleteOrphanedFiles() {
    ...
    const links =
      this.app.metadataCache.getCache(this.settings.outputFileName + ".md")
        ?.links ?? [];
  ...
  findBrokenLinks() {
    ...
    for (const sourceFilepath in brokenLinks) {
      ...
      for (const link in brokenLinks[sourceFilepath]) {
        ...
        if (
          this.settings.unresolvedLinksFileTypesToIgnore.contains(linkFileType)
        )
          continue;
  ...
  findFilesWithoutTags() {
    ...
    let withoutFiles = files.filter((file) => {
      if (
        ...
        return (
          (getAllTags(this.app.metadataCache.getFileCache(file)).length ?? 0) <=
          0
        );
      } else {
        return false;
